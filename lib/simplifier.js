...
function isSanMu(node) {
    return node.type === Syntax.ExpressionStatement && node.expression.type == Syntax.ConditionalExpression;
}
...
function isSequence(node) {
    return node.type == Syntax.ExpressionStatement && node.expression.type == Syntax.SequenceExpression;
}
...
function simplifyConditionalExpression(statementNode) {
    return {
        ...
        "test": statementNode.expression.test,
        "consequent": {
            "type": Syntax.BlockStatement,//这样拼接了一个括号
            "body": [{
                "type": Syntax.ExpressionStatement,//if 代码块只有一个语句
                "expression": isSanMu(statementNode.expression.consequent) ?
                    simplifyConditionalExpression(statementNode.expression.consequent) :
                    (isSequence(statementNode.expression.consequent) ?
                        simplifySequenceExpression(statementNode.expression.consequent) :
                        statementNode.expression.consequent)
            }]
        },
        "alternate": {
            "type": Syntax.BlockStatement,//这样拼接了一个括号
            "body": [{
                ...
                "expression": isSanMu(statementNode.expression.alternate) ?
                    simplifyConditionalExpression(statementNode.expression.alternate) :
                    (isSequence(statementNode.expression.alternate) ?
                        simplifySequenceExpression(statementNode.expression.alternate) :
                        statementNode.expression.alternate)
            }]
        }
    };
}
...
function simplifySequenceExpression(sequenceNode) {
    return {
        ...
        "body": sequenceNode.expression.expressions.map(function (el) {
            return {
                "type": Syntax.ExpressionStatement
                , "expression": isSanMu(el) ?
                    simplifyConditionalExpression(el) :
                    (isSequence(el) ? simplifySequenceExpression(el) : el)
            }
        })
    };
}
                ...
                case Syntax.ExpressionStatement:
                    if (node.expression.type === Syntax.SequenceExpression) {  //代表是一个逗号表达式
                        if (parentNode.type === Syntax.BlockStatement || parentNode.type === Syntax.Program) {
                            ...
                            /*return node.expression.expressions.map(function (el) {
                             return {
                             "type": Syntax.ExpressionStatement
                             , "expression": el
                             }
                             });*/
                            return simplifySequenceExpression(node).body;
                        } else if (parentNode.type == Syntax.IfStatement) {//if语句里面的逗号表达式
                            return simplifySequenceExpression(node);
                            /*{
                             "type": Syntax.BlockStatement,
                             "body": node.expression.expressions.map(function (el) {
                             return {
                             "type": Syntax.ExpressionStatement
                             , "expression": el
                             }
                         ...
                         */
                        return simplifyConditionalExpression(node);
