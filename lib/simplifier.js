...
function isLogicalStatement(node) {
    return node.type == Syntax.ExpressionStatement && node.expression.type == Syntax.LogicalExpression;
}
...
function simplifyLogicStatement(statementNode) {
   /* if(statementNode.expression.left.type != Syntax.LogicalExpression){
    ...
    //TODO
    if(statementNode.expression.operator === '||'){
        //左
    }
    ...
    return {
        ...
        "consequent": {
            ...
            "body": [{
                ...
                "expression": simplifyStatement(statementNode.expression.consequent)
            }]
        },
        "alternate": {
            "type": Syntax.BlockStatement,//这样拼接了一个括号
            "body": [{
                ...
                "expression": simplifyStatement(statementNode.expression.alternate)
            }]
        }
    };
}
