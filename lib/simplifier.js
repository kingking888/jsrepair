    ...
    return {
        ...
        "consequent": {
            ...
            "body": [{
                ...
                "expression": simplifyStatement(statementNode.consequent)
            }]
        },
        "alternate": {
            "type": Syntax.BlockStatement,//这样拼接了一个括号
            "body": [{
                ...
                "expression": simplifyStatement(statementNode.alternate)
            }]
        }
    };
}
...
function simplifySequenceExpression(sequenceNode) {
    return {
        ...
        "body": sequenceNode.expressions.map(function (el) {
            return {
                ...
                case Syntax.ExpressionStatement:
                    if (node.expression.type === Syntax.SequenceExpression) {  //代表是一个逗号表达式
                        if (parentNode.type === Syntax.BlockStatement || parentNode.type === Syntax.Program) {
                            ...
                            return simplifySequenceExpression(node.expression).body;
